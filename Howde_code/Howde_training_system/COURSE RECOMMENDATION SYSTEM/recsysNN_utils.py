    ...
    with open('./data/content_movie_list.csv', newline='') as csvfile:
        ...
        # Check if the line has the expected number of columns
            if count==0:
                ...
                if len(line) >= 3:  # Assuming you expect at least 3 columns (movieId, title, genres)
                    count += 1
                    movie_id = int(line[0])
                    movie_dict[movie_id] = {"title": line[1], "genres": line[2]}
                else:
                    print(f"Skipping line {count} due to insufficient columns.")        
...
def print_pred_movies(y_p, item, movie_dict, maxcount=10):
    ...
    count = 0
    disp = [["y_p", "course id", "rating ave", "course name", "skills"]]
